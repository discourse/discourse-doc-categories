...
const COLLAPSIBLE_POST_SELECTOR =
  ".topic-area .topic-post[data-post-number]:not([data-post-number='1'])," +
  ".topic-area .small-action[data-post-number]:not([data-post-number='1'])," +
  ".topic-area .gap[data-post-number]:not([data-post-number='1'])," +
  ".topic-area .time-gap[data-post-number]:not([data-post-number='1'])";
const TIMELINE_SELECTOR = ".topic-navigation";
...
export default class DocCommentsToggle extends Component {
  ...
  toggleComments() {
    ...
    if (!this.isCollapsed) {
      ...
      postStream?.refresh?.({ refreshInPlace: true });
  ...
  #syncBodyClass() {
    ...
    this.#setPostVisibility(!this.isCollapsed);
    this.#setTimelineVisibility(!this.isCollapsed);
  }
  ...
  #setPostVisibility(show) {
    if (typeof document === "undefined") {
      return;
    }

    document.querySelectorAll(COLLAPSIBLE_POST_SELECTOR).forEach((element) => {
      if (show) {
  ...
  #setTimelineVisibility(show) {
    ...
    document.querySelectorAll(TIMELINE_SELECTOR).forEach((element) => {
